...
export function UserDetailsTab({ players }: UserDetailsTabProps) {
  ...
  const filteredData = players.filter((p) => {
    // Filter by category
    if (selectedCategory === "comAgente" && !isValidId(p.agentPpPokerId))
      return false;
    if (selectedCategory === "semAgente" && isValidId(p.agentPpPokerId))
      return false;
        ...
        <span className="text-muted-foreground">
          ...
          <span
            className={cn(
              ...
              totalChipBalance >= 0 ? "text-[#00C969]" : "text-[#FF3638]",
            )}
          >
      ...
      <div className="border-t border-border/40 py-2">
        <div className="flex items-center gap-3 flex-wrap text-xs">
          ...
          <button
            ...
            onClick={() =>
              setSelectedCategory(
                selectedCategory === "comAgente" ? null : "comAgente",
              )
            }
            className={cn(
          ...
          >
            ...
            <span
              className={cn(
                "font-mono text-[10px]",
                chipBalanceWithAgent >= 0 ? "text-[#00C969]" : "text-[#FF3638]",
              )}
            >
          ...
          <button
            ...
            onClick={() =>
              setSelectedCategory(
                selectedCategory === "semAgente" ? null : "semAgente",
              )
            }
            className={cn(
              "flex items-center gap-1.5 px-2 py-1 rounded transition-colors hover:bg-muted/50",
      ...
      <div className="border-t border-border/40 pt-2 pb-4">
        <div className="overflow-x-auto">
          <table
            className={cn(
              "w-full text-xs",
              expanded ? "min-w-[1400px]" : "min-w-[700px]",
            )}
          >
            <thead>
