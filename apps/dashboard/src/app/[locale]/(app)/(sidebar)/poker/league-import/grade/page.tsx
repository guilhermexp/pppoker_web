...
function getWeekFromDateString(dateStr: string, year?: number): number | null {
  try {
    ...
    // Formato DD/MM
    const date = parse(`${dateStr}/${currentYear}`, "dd/MM/yyyy", new Date(), { locale: ptBR });
...
function GameTypeBadge({ game }: { game: string }) {
  ...
  return (
    <Badge variant="outline" className={`text-[10px] font-medium ${colorClass}`}>
      {game}
    </Badge>
  );
}
    ...
    <div className="border rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-muted/30 border-b">
        ...
        <span className="text-sm text-muted-foreground">
          GTD: <span className="font-medium text-foreground">{formatNumber(dayTotal)}</span>
        </span>
      </div>
        ...
        <TableBody>
          {events.map((event) => (
            <TableRow key={event.id} className="text-sm">
              ...
              <TableCell className="font-medium max-w-[200px] truncate" title={event.name}>
                {event.name}
              </TableCell>
              <TableCell>
...
export default function TournamentSchedulePage() {
  ...
  const totals = data ? calculateTotals(data.events) : { totalGTD: 0, totalTournaments: 0 };
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/30 border">
              ...
              <span className="text-sm">
                <span className="font-bold">{formatNumber(totals.totalTournaments)}</span> torneios
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20">
              ...
              <span className="text-sm text-green-600">
                GTD Total: <span className="font-bold">{formatNumber(totals.totalGTD)}</span>
              </span>
            </div>

            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/30 border">
              <span className="text-sm text-muted-foreground">
                R$ <span className="font-bold text-foreground">{formatNumber(totals.totalGTD * 5)}</span>
          ...
          <div className="relative z-20 m-auto flex w-full max-w-[380px] flex-col">
            <div className="flex w-full flex-col relative text-center">
              ...
              <p className="pb-6 text-sm text-[#878787]">
                Faça upload da planilha XLSX com a programação semanal de torneios PPST.
              </p>
