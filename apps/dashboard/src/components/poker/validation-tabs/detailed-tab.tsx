...
export function DetailedTab({ detailed, summaries = [] }: DetailedTabProps) {
  ...
  const uniqueAgents = new Set(detailed.map((d) => d.agentPpPokerId).filter(isValidId));
  const uniqueSuperAgents = new Set(detailed.map((d) => d.superAgentPpPokerId).filter(isValidId));
  ...
  const playersLeftClub = detailed
    ...
    .map((d) => ({ id: d.ppPokerId, nickname: d.nickname, balance: d.creditLeftClub }));
  ...
  return (
    <div className="space-y-3 pb-4">
      ...
      {onlyInDetailed.length > 0 && (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <Icons.ChevronDown className={cn("w-3 h-3 text-amber-500 transition-transform", showDifference && "rotate-180")} />
          </div>
        </div>
      )}
              ...
              <tbody className="divide-y divide-border/30">
                {onlyInDetailed.map((d, idx) => (
                  <tr key={`${d.ppPokerId}-${idx}`}>
                    <td className="py-1 font-mono text-muted-foreground text-[10px]">{d.ppPokerId}</td>
                    <td className="py-1">{d.nickname}</td>
                    <td className={cn("py-1 text-right font-mono text-[10px]", d.totalWinnings >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
      ...
      <div className="flex items-center gap-4 text-xs py-2">
        ...
        <span className="text-border/60">·</span>
        <span className="text-muted-foreground">
          Mãos <span className="text-foreground font-medium">{formatNumber(totalHands)}</span>
        </span>
        <span className="text-border/60">·</span>
        <span className="text-muted-foreground">
          Tipos Ativos <span className="text-foreground font-medium">{activeGameTypes.length}</span>
        </span>
      </div>
      ...
      <div className="border-t border-border/40 flex items-center gap-4 text-xs py-2">
        <span className="text-muted-foreground">
          Total{" "}
          <span className={cn("font-mono font-medium", totalWinnings >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
        ...
        <span className="text-muted-foreground">
          Ganhos+Eventos{" "}
          <span className={cn("font-mono font-medium", totalGeneralPlusEvents >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
        ...
        <span className="text-muted-foreground">
          Jackpot{" "}
          <span className={cn("font-mono font-medium", totalJackpot >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
        ...
        <span className="text-muted-foreground">
          EV Split{" "}
          <span className={cn("font-mono font-medium", totalEvSplit >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
      ...
      <div className="border-t border-border/40 flex items-center gap-4 text-xs py-2">
        <span className="text-muted-foreground">
          Fichas Env.{" "}
          <span className="font-mono font-medium text-foreground">{formatNumber(totalChipSent)}</span>
        </span>
        <span className="text-border/60">·</span>
        <span className="text-muted-foreground">
          Fichas Resg.{" "}
          <span className="font-mono font-medium text-foreground">{formatNumber(totalChipRedeemed)}</span>
        </span>
        <span className="text-border/60">·</span>
        <span className="text-muted-foreground">
          Crédito Env.{" "}
          <span className="font-mono font-medium text-foreground">{formatCurrency(totalCreditSent)}</span>
        </span>
        <span className="text-border/60">·</span>
        <span className="text-muted-foreground">
          Crédito Resg.{" "}
          <span className="font-mono font-medium text-foreground">{formatCurrency(totalCreditRedeemed)}</span>
        </span>
        {totalCreditLeftClub !== 0 && (
          <>
            ...
            <span className="text-muted-foreground">
              Jog. Saiu{" "}
              <span className={cn("font-mono font-medium", totalCreditLeftClub < 0 ? "text-[#FF3638]" : "text-[#00C969]")}>
                {formatCurrency(totalCreditLeftClub)}
              </span>
              <span className="text-[10px] text-muted-foreground ml-1">({playersLeftClub.length})</span>
            </span>
          </>
        )}
        {totalCreditLeftClub2 !== 0 && (
          <>
            ...
            <span className="text-muted-foreground">
              Ag. Saiu{" "}
              <span className={cn("font-mono font-medium", totalCreditLeftClub2 < 0 ? "text-[#FF3638]" : "text-[#00C969]")}>
                {formatCurrency(totalCreditLeftClub2)}
              </span>
              <span className="text-[10px] text-muted-foreground ml-1">({agentsLeftClub.length})</span>
            </span>
          </>
        )}
      </div>
          ...
          {GAME_CATEGORIES.map((category) => {
            ...
            return (
              ...
              >
                ...
                <span className={cn("font-mono", categoryTotal >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
      ...
      {gameTypesExpanded && selectedCategory && (
        <div className="border-t border-border/40 py-2">
          {GAME_CATEGORIES.filter((c) => c.name === selectedCategory).map((category) => (
            <div key={category.name} className="flex items-center gap-4 flex-wrap text-xs">
              ...
              {category.games.map((game) => {
                ...
                return (
                  <span
                    key={game.key}
                    className={cn("text-muted-foreground", !isActive && "opacity-40")}
                  >
                    <span className="text-[9px] font-mono">({game.col})</span> {game.label}{" "}
                    {isActive && (
                      <span className={cn("font-mono", gameTotal >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
      ...
      <div className="border-t border-border/40 flex items-center justify-between py-2">
        <div className="flex items-center gap-3">
          <span className="text-xs text-muted-foreground">{detailed.length} registros</span>
          <Button
            variant="ghost"
            size="sm"
      ...
      <div className="border-t border-border/40 pt-2 pb-4">
        <div className="overflow-x-auto">
          <table className={cn("w-full text-xs", expanded ? "min-w-[4000px]" : "min-w-[1000px]")}>
            <thead>
                ...
                {columns.map((col) => (
                  ...
                  >
                    {col.key === "ppPokerId" ? "TOTAL" :
                     col.key === "nickname" ? `${filteredData.length} jog.` :
