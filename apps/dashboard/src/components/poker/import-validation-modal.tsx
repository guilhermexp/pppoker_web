...
export function ImportValidationModal({
  ...
  const { qualityScore, passedChecks, totalChecks, hasBlockingErrors, checks } = validationResult;
  ...
  const clubId = parsedData.transactions?.find((t) => t.senderClubId)?.senderClubId || null;

  // Calculate week numbers
  const weekInfo = useMemo(() => {
    const currentWeek = getWeek(new Date(), { weekStartsOn: 0, firstWeekContainsDate: 1 });
      ...
      <DialogContent className="max-w-[95vw] w-[1600px] max-h-[92vh] overflow-hidden flex flex-col p-0 bg-background border border-border shadow-2xl rounded-xl">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
          <TabsList className="flex-shrink-0 w-full justify-start border-b border-border bg-transparent h-auto px-4 py-3 gap-1">
            ...
            >
              {hasBlockingErrors && <Icons.AlertCircle className="w-3.5 h-3.5 mr-1" />}
              Validação ({passedChecks}/{totalChecks})
            </TabsTrigger>
          </TabsList>

          <div className="flex-1 overflow-hidden px-6 py-4 bg-muted/20 min-h-[500px]">
            <TabsContent value="resumo" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <ResumoTab
            ...
            <TabsContent value="general" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <GeneralTab summaries={parsedData.summaries || []} />
            </TabsContent>

            <TabsContent value="detailed" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <DetailedTab detailed={parsedData.detailed || []} summaries={parsedData.summaries || []} />
            </TabsContent>

            <TabsContent value="sessions" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <SessionsTab
                sessions={parsedData.sessions || []}
                period={validationResult.period}
            ...
            <TabsContent value="transactions" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <TransactionsTab transactions={parsedData.transactions || []} />
            </TabsContent>

            <TabsContent value="demonstrativo" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <DemonstrativoTab demonstrativo={parsedData.demonstrativo || []} />
            </TabsContent>

            <TabsContent value="user-details" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <UserDetailsTab players={parsedData.players || []} />
            </TabsContent>

            <TabsContent value="rakeback" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <RakebackTab rakebacks={parsedData.rakebacks || []} />
            </TabsContent>

            <TabsContent value="cadastro" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
              <CadastroTab summaries={parsedData.summaries || []} />
            </TabsContent>

            <TabsContent value="validation" className="mt-0 h-full min-h-[460px] rounded-lg bg-background p-4 pb-8 shadow-sm overflow-y-auto max-h-[calc(92vh-240px)]">
