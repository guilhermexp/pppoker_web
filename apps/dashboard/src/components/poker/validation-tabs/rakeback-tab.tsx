...
export function RakebackTab({ rakebacks }: RakebackTabProps) {
  ...
  const uniqueSuperAgents = new Set(
    rakebacks
      ...
      .filter((id) => id && id.toLowerCase() !== "(none)" && id.toLowerCase() !== "none")
  );
  ...
  const byAgent = rakebacks.reduce((acc, r) => {
    ...
    acc[id].count += 1;
    return acc;
  }, {} as Record<string, { id: string; nickname: string; totalRt: number; avgPercent: number; count: number }>);
    ...
    <div className="space-y-3 pb-4">
      ...
      <div className="flex items-center gap-4 text-xs py-2">
        ...
        >
          ...
          <Icons.ChevronDown className={cn("w-3 h-3 inline ml-1 transition-transform", showByAgent && "rotate-180")} />
        </button>
        <span className="text-border/60">·</span>
        <span className="text-muted-foreground">
      ...
      {showByAgent && agentsList.length > 0 && (
        <div className="border-t border-border/40 py-2">
          <div className="flex items-center gap-4 flex-wrap text-xs">
            <span className="text-muted-foreground text-[10px] font-medium">Top agentes:</span>
            {agentsList.slice(0, 6).map((agent) => (
              <span key={agent.id} className="text-muted-foreground">
                {agent.nickname}{" "}
                <span className={cn("font-mono", agent.totalRt >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
                  {formatCurrency(agent.totalRt)}
                </span>
                <span className="text-[9px] text-muted-foreground ml-1">({formatPercent(agent.avgPercent)})</span>
              </span>
            ))}
            {agentsList.length > 6 && (
              <span className="text-[10px] text-muted-foreground">+{agentsList.length - 6} mais</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="border-t border-border/40 flex items-center gap-4 text-xs py-2">
        <span className="text-muted-foreground">
          Total RT{" "}
          <span className={cn("font-mono font-medium", totalRt >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
        ...
        <span className="text-muted-foreground">
          Média RT{" "}
          <span className="font-mono font-medium text-foreground">{formatPercent(avgRakeback)}</span>
        </span>
      </div>
          ...
          <table className="w-full text-xs min-w-[700px]">
            <thead>
              <tr className="text-muted-foreground border-b border-border/40">
                <th className="py-1.5 px-2 font-medium text-left whitespace-nowrap">ID Superagente</th>
                <th className="py-1.5 px-2 font-medium text-left whitespace-nowrap">ID Agente</th>
                ...
                <th className="py-1.5 px-2 font-medium text-right whitespace-nowrap">Retorno %</th>
                <th className="py-1.5 px-2 font-medium text-right whitespace-nowrap">Total RT</th>
              </tr>
            </thead>
              ...
              {filteredData.length === 0 ? (
                ...
                filteredData.map((row, index) => (
                  <tr key={`${row.agentPpPokerId}-${index}`} className="hover:bg-muted/30">
                    ...
                    <td className="py-1.5 px-2 text-muted-foreground whitespace-nowrap">{row.country || "-"}</td>
                    <td className="py-1.5 px-2 whitespace-nowrap">{row.agentNickname}</td>
                    ...
                    <td className={cn("py-1.5 px-2 text-right font-mono whitespace-nowrap", row.totalRt >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
            ...
            {filteredData.length > 0 && (
              <tfoot>
                <tr className="border-t-2 border-border font-medium bg-muted/30">
                  <td colSpan={5} className="py-1.5 px-2">TOTAL</td>
                  <td className="py-1.5 px-2 text-right font-mono">
                    {formatPercent(
                      filteredData.reduce((sum, r) => sum + r.averageRakebackPercent, 0) / filteredData.length
                    )}
                  </td>
                  <td className={cn("py-1.5 px-2 text-right font-mono", filteredData.reduce((sum, r) => sum + r.totalRt, 0) >= 0 ? "text-[#00C969]" : "text-[#FF3638]")}>
                    {formatCurrency(filteredData.reduce((sum, r) => sum + r.totalRt, 0))}
                  </td>
                </tr>
              </tfoot>
            )}
