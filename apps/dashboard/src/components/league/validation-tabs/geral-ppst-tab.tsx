...
export function LeagueGeralPPSTTab({ data }: LeagueGeralPPSTTabProps) {
  ...
  const brCount = data.filter(bloco => LIGAS_BR.includes(bloco.contexto.entidadeId)).length;
                ...
                <Table>
                  ...
                  <TableBody>
                    {(() => {
                      ...
                      const ligasBR = bloco.ligas.filter(l => LIGAS_BR.includes(l.ligaId));
                      const ligasOutras = bloco.ligas.filter(l => !LIGAS_BR.includes(l.ligaId));

                      const renderLigaRow = (liga: typeof bloco.ligas[0], ligaIndex: number) => (
                        ...
                        <>
                          ...
                          {ligasBR.length > 0 && (
                            <TableRow className="bg-blue-500/5">
                              <TableCell colSpan={14} className="py-1 text-[10px] font-medium text-blue-600">
                                🇧🇷 Brasil (60%)
                              </TableCell>
                            </TableRow>
                          )}
                          ...
                          {/* Demais ligas */}
                          {ligasOutras.map((liga, i) => renderLigaRow(liga, ligasBR.length + i))}
                        </>
                      );
                    })()}
                    {/* Total Row */}
