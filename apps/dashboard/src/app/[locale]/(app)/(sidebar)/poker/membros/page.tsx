...
export default async function PokerMembrosPage() {
  ...
  // Prefetch stats (getLive calls bridge so no SSR prefetch for it)
  try {
    await queryClient.fetchQuery(trpc.poker.members.getStats.queryOptions());
  } catch {
  ...
  return (
    <HydrateClient>
      <div className="flex flex-col gap-6">
        <div className="pt-6">
          <h1 className="text-2xl font-medium">{t("poker.members.title")}</h1>
